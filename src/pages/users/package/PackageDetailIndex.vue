<template>
  <q-page class="q-py-md">
        ...
        <div class="q-py-md payment-container">
          <div class="payment-main q-px-md q-mb-md">
            <div class="q-mb-sm">
              ...
              >
                <h3
                  class="flex-1 title q-mx-none q-pl-none q-pr-sm"
          ...
          <div class="payment-sidebar q-px-md">
            <q-card class="q-pa-sm q-mb-md">
              <div>
                ...
                <div class="q-mb-sm" v-if="discount !== null">
                  <div
                    class="q-mx-sm"
                    :style="
                      (discount.isValid
                        ? 'border: 1px solid #0A929E;'
                  ...
                  <div
                    ...
                    class="text-negative q-px-sm text-light q-mt-xs text-italic"
              ...
              <div
                v-if="
                  !discount || price.subTotal === 0 || paymentGateway === null
                "
              >
                <div v-if="price.subTotal > 0" class="q-mb-sm">
                  <q-btn
                    v-if="price.total > 0"
                    push
                    dense
                    ...
                    :id="'choosePaymentMethods-' + aPackage.id"
                  ...
                  >
                    ...
                    <q-btn
                      ...
                      @click="
                        $refs.dialogHowToPay.show(
                          paymentGatewayMethodCode,
                          paymentGateway.code
                        )
                      "
                    />
              ...
              <div v-else>
                <div v-if="price.total > 0" class="q-mb-sm">
                  <q-btn
                    push
                    dense
                    no-caps
                  ...
                  <div class="payment-method-item" v-if="paymentGateway">
                    <template v-if="paymentGateway.img">
                      <img
                        :class="paymentGateway.img ? '' : 'hidden'"
                        :src="paymentGateway.img"
              ...
              <div v-if="paymentGatewayMethodId == 102">
                <!-- Ewallet -->
                <q-input
                  outlined
                  dense
                ...
                <q-input
                  ...
                  :error-message="
                    $utils.isEmpty(user.phone) ? 'Harap isi nomor ponsel' : null
                  "
                />
              </div>
              <div class="row q-px-sm">
                ...
                <div>Rp {{ price.subTotal.toLocaleString() }}</div>
              </div>
              ...
              <div align="right" class="q-mt-md">
                <div>
                  <q-btn
                    ...
                    :loading="loading"
                    :disable="
                      (paymentGateway == null && price.total > 0) ||
                      (paymentGatewayMethodId == 102 &&
                        $utils.isEmpty(user.phone))
                    "
                    class="q-px-sm"
                    color="primary"
                    :label="price.total > 0 ? 'Lanjut Pembayaran' : 'Lanjut'"
                    :id="'continuePaymentButton-' + aPackage.id"
                    @click="onConfirm"
                  />
                </div>
                <div>
                  <error v-if="paymentGateway == null && price.total > 0"
                    >Harap pilih metode pembayaran</error
                  >
                </div>
              </div>
            </q-card>
    ...
    <DialogPromoList
      ...
      "
    />
    <DialogHowToPay ref="dialogHowToPay" />
...
export default {
  ...
  methods: {
    getPackages() {
      this.$services.package.get(
        { product_type: 0, is_active: 1 },
        (data) => {
          for (var i in data) {
            const item = data[i];
    ...
    onConfirm() {
      ...
      this.loading = true;
      this.$services.product.purchase(
        form,
        (data) => {
          this.$router.push({
    ...
    purchaseSimulate() {
      ...
      this.$services.product.purchaseSimulate(
        form,
        (data) => {
          this.price.discount = data.discount;
          this.price.total =
            data.price_sell + data.unique_number - this.price.discount;
        },
        (msg, errors) => {
          this.$q.notify({
            message: msg,
    ...
    formPurchase: function (bypass) {
      ...
      return {
        ...
        packages: [this.aPackage ? this.aPackage.id : null],
      };
    },
    ...
    checkCoupon() {
      this.loading = true;
      this.$services.coupon.getDetail(
        this.couponCode,
        (data) => {
          data.isValid = true;
          this.discount = data;
    ...
    checkCouponValidation() {
      if (this.discount) {
        let { isValid, invalidMessage } = this.$utils.validateCoupon(
          this.discount,
          this.price.subTotal,
          [this.aPackage]
        );
