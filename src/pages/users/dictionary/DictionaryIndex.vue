<template>
  <q-page>
    <section
      id="section-head"
      class="bg-dark q-pa-md flex flex-center">
      <div class="container text-center q-py-lg">
        ...
        <q-select
          outlined dark dense
          use-input hide-selected fill-input emit-value map-options
          color="secondary"
          class="q-mx-auto"
    ...
    <section class="container-sm q-pa-md">
      ...
      <q-card class="q-mb-lg">
        <div class="bg-secondary-light text-dark q-pa-md"
          style="display: inline-block; left: 0; top: 0; border-radius: 0; border-bottom-right-radius: 16px; font-size: 24px; font-weight: bold;">
        ...
        <div class="q-pa-md row dictionary-container">
          <a
            ...
            :key="'data-' + idx"
            no-caps flat dense class="block col-md-4 q-my-xs app-link">{{dictionary.name}}</a>
        </div>
      </q-card>
    </section>
  </q-page>
...
export default {
  ...
  methods: {
    ...
    onSearchSelected (value) {
      ...
      this.$router.push({
        path: this.$route.params.alphabet + "/" + this.$utils.escapeRoute(value)
      })
    },
