<template>
  <div :class="'item-product-learnpath ' + (active ? 'active' : '')">
    <div :class="'--progress-before ' + (activeBefore ? 'active' : '')"></div>
    <div :class="'--progress-after ' + (activeNext ? 'active' : '')"></div>
    <div class="relative">
      <a
        :href="$utils.getContentRoute(product)"
        @click="
          (e) => {
            $utils.openLink(e, $utils.getContentRoute(product), $router);
          }
        "
      >
    ...
    <div class="--content flex-1">
      <div class="relative">
        ...
        <div class="--rating flex">
          ...
          <div class="--rating-people">({{ getRatingCnt() }})</div>
        </div>
        <p class="--description">
          {{ product.description }}
        </p>
      ...
      <div>
        <span class="--price"
          >Rp {{ product.price_sell.toLocaleString() }}</span
        >
        <span
          v-if="
            (product.price_original || product.price_ori) != product.price_sell
          "
          class="--price-discount"
          >{{
            (product.price_original || product.price_ori).toLocaleString()
          }}</span
        >
      </div>
    </div>
