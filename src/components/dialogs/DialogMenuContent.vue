  ...
  <div ref="sidebar">
    ...
    <div class="sidebar-list-group">
      <q-btn class="sidebar-list-group-bottom" unelevated rounded no-caps v-for="item in navs.materi"
        :style="{ backgroundColor: localMenu == item.id ? '#FFDE59' : '' }" :key="item.id"
        @click="handleButtonClick(item.id, 'materi')">
        {{ item.code }}
      </q-btn>
    </div>
    ...
    <div class="sidebar-list-group">
      <q-btn class="sidebar-list-group-bottom" unelevated rounded no-caps v-for="item in navs.pekerjaan" :key="item.id"
        :class="{ 'active': localMenu === item.id }" @click="handleButtonClick(item.id, 'pekerjaan', true)">
        {{ item.code }}
      </q-btn>
    </div>
    <!-- Button "Terapkan" -->
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      type: null,
      activeId: null,
      localMenu: this.activeMenuLocal,
      temp: 3
    };
  },
  mounted() {
    this.localMenu = localStorage.getItem('localActive');
  },
