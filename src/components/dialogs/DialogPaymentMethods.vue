  ...
  <q-dialog v-model="dialog">
    <q-card class="q-pa-md" style="width: 600px; height: 100%; max-width: calc(100% - 32px)">
      ...
      <q-card-section class="q-pa-none q-ma-xs">
        <div
          v-for="method in Object.keys(payment.methods)"
          :key="method">
          <div
            ...
            <div
              :class="'payment-method-item ' + (['BCA', 'SAHABAT_SAMPOERNA', 'BJB', 'CIMB', 'BSI'].indexOf(item.code) > -1 ? 'hidden' : '')"
  ...
  methods: {
    ...
    getPaymentMethods (method) {
      this.$services.payment.gateway(method, (data) => {
        ...
        this.payment.methods[method].list = data
      }, (msg, errors) => {
      }, () => {
      })
    },
