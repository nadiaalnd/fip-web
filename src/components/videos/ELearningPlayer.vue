<template>
  <div class="q-py-md">
    ...
    >
      ...
      >
        <ImageComponent :src="product.thumbnail" width="100%" height="100%" />
      </div>
    </div>
    <h2 class="text-primary q-mb-md">Deskripsi</h2>
    <p v-html="product.description"></p>
    <div v-if="product.is_paid == 1" class="q-mb-md">
      <h2 class="text-primary q-mb-md">Progress</h2>
      <p>
        <q-linear-progress
          rounded
        ...
        >
          <q-tooltip> {{ product.percentage }} % </q-tooltip>
        </q-linear-progress>
      </p>
      <div class="text-center">
        ...
        >
          <q-tooltip> Minimal 80% menonton dari durasi product </q-tooltip>
        </q-btn>
      </div>
    </div>
    <div class="price text-center" v-if="product.is_paid == 0">
      <q-btn
        push no-caps
    ...
    <DialogContentPayment ref="dialogContentPayment" />
    <DialogShare :model="product" :url="$route.path" ref="dialogShare" />
    <DialogRatingProduct
    ...
    />
    <DialogComment ref="dialogComment" />
  </div>
</template>
<script>
...
import DialogComment from "components/dialogs/DialogComment.vue";
...
export default {
  ...
  methods: {
        ...
        (data) => {
          ...
          if (data.url) {
            this.$nextTick(() => {
              ...
              this.$refs.videoPlayer.setVideo(
                this.product.id,
                `${process.env.API_URL_VIDEO}/vod${this.product.url}/playlist.m3u8`
              );
            });
          }
    ...
    getPurchasedProduct() {
      ...
      this.$services.product.purchaseList(
        this.page,
        "1",
        (data) => {
          for (var i in data) {
    ...
    downloadCertificatePdf() {
      window.location.assign(
        process.env.API_URL + this.certificatePath,
        "_blank"
      );
    },
    ...
    updatePercentage(pointer, duration) {
      ...
      if (pointer <= duration) {
        ...
        this.product.percentage =
          Math.round((pointer / duration) * 10000) / 100;
      } else {
